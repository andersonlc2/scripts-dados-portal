...
tmp="tmp_bolsa_familia"
final="final_bolsa_familia"
db="amlcpgf"
user="root"
...
create_db_and_tables()
{
    psql -U $user $db < arch_sql/create_tables_bolsa_familia.sql
}
...
insert_table_final()
{
    psql -U $user $db < arch_sql/insert_final_bolsa_familia.sql
}
...
donwload_import()
{
    ...
    url="https://www.portaltransparencia.gov.br/download-de-dados/bolsa-familia-pagamentos/$1"
    wget "$url"
    unzip "$1"
    path=$(pwd)
    psql -U $user $db --command="
        COPY $tmp  FROM '${path}/${1}_BolsaFamilia_Pagamentos.csv' CSV DELIMITER ';' HEADER ENCODING 'latin1';"
    rm -rf "${1}"*
}
